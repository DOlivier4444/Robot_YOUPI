...
def Serial_Synchronisation(arduino: serial.Serial) -> None: # Envoie un message de synchronisation à l'Arduino et attend la réponse.
  while True:
    # Envoi du message
    print(f"Envoi du message de synchronisation : {Comms.To_Arduino.RASPBERRY_READY.name}")
    Send_To_Arduino(arduino, Comms.To_Arduino.RASPBERRY_READY.value)
    ...
    if message is None:
      ...
      if message == Comms.From_Arduino.ARDUINO_READY.value:
        print(f"Réception du message de synchronisation : {Comms.From_Arduino.ARDUINO_READY.name}")
        break
...
def Open_xlsx_WorkBook(programFileName:str) -> openpyxl.Workbook: # Open an excel workbook, mount it, and return it
  print(f"\nOuverture du fichier de programmes {programFileName}...")
...
def Program_Choice(workBook:openpyxl.Workbook) -> openpyxl.worksheet.worksheet.Worksheet: #List the programs in the workbook and let the user choose one
  ...
  available_sheets = workBook.sheetnames[2:]  # skip the first two sheets
  ...
  while True:
    ...
    choix = input("Choix du programme : ")
    if choix.isdigit() and 1 <= int(choix) <= len(available_sheets):
      selection = available_sheets[int(choix) - 1]
      print(f"Vous avez choisi : {selection}")
      break  # ou continue si tu veux répéter après traitement
    else:
      print(f"Choix invalide, veuillez faire un choix entre 1 et {len(available_sheets)}.")

  workSheets = workBook.worksheets[int(choix)+1] # workSheet
  return workSheets
...
def Get_Program(workSheets:openpyxl.worksheet.worksheet.Worksheet) -> list | None:  # Read all the row of a program to create a 2d array of it
  ...
  nbrOfmovements = Read_cell_value(workSheets, row=1, column=1)
  try :
...
def Program_Execution(arduino:serial, workSheets:openpyxl.worksheet.worksheet.Worksheet) -> int:  # Execute the program by sending the movements datas to the arduino
  ...
  print(Comms.To_Arduino.PROGRAM_EXECUTION.name)
  ...
  while mvtNbr < len(program) :
    print(f"--- Mouvement no {mvtNbr+1} sur {len(program)} ---")

    # convert from raw XLS format to joint format (Speed, J1, J2, ..., J5, gripper)
    print(f"Mouvement brut fichier excel : {program[mvtNbr]}")
    movementJoint = Convert_movement_to_joint(program[mvtNbr])
    print(f"mouvement convertis : {movementJoint}")
    ...
    print(f"string à envoyer : {movementString}")

    # wait for the arduino to be ready to receive the movements datas
    arduino.flushInput()
    start = time.time()
    while (Recv_From_Arduino(arduino)) != Comms.From_Arduino.READY_TO_RECEIVE.value:
      if time.time() - start > Comms.General.TIMEOUT:
        raise TimeoutError("Arduino n'a pas envoyé READY_TO_RECEIVE dans le temps imparti.")
    print(Comms.From_Arduino.READY_TO_RECEIVE.name)

    #if abortProgram == True :
    #Send_To_Arduino(arduino, Comms.To_Arduino.ABORT_PROGRAM.value)

    # send the movement datas to the arduino
    Send_To_Arduino(arduino, Comms.To_Arduino.START_MOVEMENT.value, movementString)
    time.sleep(0.1) # give some time to the arduino to process
    ...
    arduino.flushInput()
    print("En attente de la fin du mouvement...")
    while (Recv_From_Arduino(arduino)) != Comms.From_Arduino.MOVEMENT_FINISHED.value:
      #Send_To_Arduino(arduino, '1') # send a constant value to the arduino to indicate that no abort is requested
      pass # wait for the movement to be finished
    print(Comms.From_Arduino.MOVEMENT_FINISHED.name)

    mvtNbr = mvtNbr + 1

  Send_To_Arduino(arduino, Comms.To_Arduino.PROGRAM_FINISHED.value)
  print(Comms.To_Arduino.PROGRAM_FINISHED.name)
