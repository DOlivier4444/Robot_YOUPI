...
def Read_cell_value(workSheets:openpyxl.worksheet.worksheet.Worksheet, row:int, column:int) -> str | None:
  return workSheets.cell(row = row, column = column).value
...
def Get_Movement_Datas(workSheets:openpyxl.worksheet.worksheet.Worksheet, row_no:int) -> list[str] | None:
  ...
  for i in range(startColumn, startColumn + nbrOfData):
    ...
    if cell_value is None or not isinstance(cell_value, float):
...
def Get_Program_Datas(workSheets:openpyxl.worksheet.worksheet.Worksheet) -> list[str] | int:
  ...
  while (True) :
    ...
    mvtDatas = Get_Movement_Datas(workSheets, rowNbr)
    if mvtDatas is None :
      return 0xD4
    rowNbr += 1
    ...
    if mvtDatas[0] == Youpi.Movement_Mode.LINEAR.value :
      motorAnglesTargets = Inverse_Kinematic(
        ...
        penOffsetV  = float(mvtDatas[7]),
        penOffsetH  = float(mvtDatas[8])
      )
    else : # joint
...
def Program_Execution(arduino:serial, workSheets:openpyxl.worksheet.worksheet.Worksheet) -> int:
  ...
  # Send a message to say to the ino that a movement is about to start ?
  i = 0
  while (True) :
    if i >= len(mvtsJointDatas) : # i >= nbrOfMovements
      break
    # wait for the message 'ready to receive'


    #elif abortProgram == True :
    #  # Send a stop message to the raspberry
    #  pass
    ...
    if receivedData == Comms.From_Arduino.MOVEMENT_FINISHED.value :
      i = i + 1
    elif receivedData == Comms.General.ERROR_MOVEMENT.value :
      return receivedData
